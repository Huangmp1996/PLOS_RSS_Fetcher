...
def analyze_article_with_llm(title, summary, journal_title=""):
    """采用 CoT 思维链与迁移可信度机制的生态学/行为学/交叉启发筛选"""
    if not client:
        return None

    system_prompt = (
        "你是一名兼具宏观生态学、保护科学、行为学与计算科学背景的资深科研评估专家。"
        "你的任务是精准识别生态学/行为学直接研究，以及真正具备数学/统计同构价值的跨学科通用方法，"
        "坚决剔除缺乏可落地性的概念隐喻与微观机制强行类比。"
    )
    
    user_prompt = f"""请评估以下发表在《{journal_title}》上的学术论文：
...
【评估标准与思维链要求】
请按以下逻辑进行推导：
1. 本土研究判定：是否直接研究个体、种群、群落、生态系统、宏观演化、生物地理或动物行为学科学问题？
2. 跨学科迁移可信度（仅当不属于第 1 条时评估）：
   - 排除：纯微观生物机制（密码子、蛋白质折叠、微流控、细胞因子等具体生物物理过程）。
   - 准入：具有尺度无关性的通用数学/计算/统计方法（如复杂网络动力学、状态空间模型、非线性断裂点检测、空间点格局分析、计算机视觉/声学信号表征等）。
   - 检验：该方法是否能直接输入宏观生态/行为数据（如物种多度矩阵、轨迹数据、声音频谱、网格化环境层）进行计算？若无法落地，视为无效类比。

【输出评分标准】
- 1-2分：完全无关，或为微观机制的生硬类比。
- 3分：方法具有明确数学同构性，可潜在迁移至宏观生态/行为分析。
- 4-5分：生态学/进化/行为学直接研究，或极具突破性的通用方法。
